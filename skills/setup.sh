...
show_menu() {
    ...
    local options=("Opencode" "Claude Code" "Gemini CLI" "Codex (OpenAI)" "GitHub Copilot")
    local selected=(true false false false false)  # Opencode selected by default
...
if [ "$SETUP_OPENCODE" = false ] && [ "$SETUP_CLAUDE" = false ] && [ "$SETUP_GEMINI" = false ] && [ "$SETUP_CODEX" = false ] && [ "$SETUP_COPILOT" = false ]; then
    show_menu
    echo ""
fi
...
echo ""
echo "Configured:"
[ "$SETUP_OPENCODE" = true ] && echo "  • Opencode:       .agents/skills/ (native)"
